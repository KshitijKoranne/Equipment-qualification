...
// GET: list all requalifications for an equipment
...
// POST: add a new requalification event
export async function POST(req: NextRequest) {
  await ensureDB();
  const body = await req.json();
  const {
    equipment_id, requalification_ref, frequency, tolerance_months,
    scheduled_date, protocol_number, remarks,
  } = body;
  ...
  const result = await db.execute({
    sql: `INSERT INTO requalifications (equipment_id, requalification_ref, frequency, tolerance_months,
          scheduled_date, protocol_number, remarks, status)
          VALUES (?, ?, ?, ?, ?, ?, ?, 'Scheduled')`,
    args: [
      equipment_id, requalification_ref,
      frequency || "Annual", tolerance_months || "1",
      scheduled_date || null, protocol_number || null, remarks || null,
    ],
  });
